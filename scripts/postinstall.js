...
async function main() {
  ...
  try {
    ...
    await downloadFile('https://github.com/Brainicism/bgutil-ytdlp-pot-provider/releases/latest/download/bgutil-ytdlp-pot-provider.zip', path.join(pluginDir, 'bgutil-ytdlp-pot-provider.zip'));
    
    console.log('Installing bgutil-ytdlp-pot-provider server...');
    if (!fs.existsSync(serverDir)) {
      await fsp.mkdir(path.dirname(serverDir), { recursive: true });
      execSync(`git clone --single-branch --branch 2.0.0 https://github.com/Brainicism/bgutil-ytdlp-pot-provider.git "${serverDir}"`, { stdio: 'inherit' });
    }
